...
public:
    ...
    // Vortexa Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between vortexad, and vortexa-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Vortexa Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<vortexaaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AIVortexa/vortexa>";
    const std::string URL_WEBSITE = "<https://aivortexa.com>";
...
/** Sanity checks
 *  Ensure that Vortexa Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vortexa Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single Vortexa Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Vortexa Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Vortexa Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/AIVortexa/vortexa/pull/1817 and https://github.com/AIVortexa/vortexa/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Vortexa-specific tasks
