...
const char * const BITCOIN_CONF_FILENAME = "vortexa.conf";
const char * const BITCOIN_PID_FILENAME = "vortexad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "vortexad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VortexaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VortexaCore
    // Mac: ~/Library/Application Support/VortexaCore
    // Unix: ~/.vortexacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VortexaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VortexaCore";
#else
    // Unix
    return pathRet / ".vortexacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty vortexa.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Vortexa Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Vortexa Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2017, nEndYear) + "The Vortexa Core developers";
    }
