...
/*
    ...
    allow endless transaction that would bloat Vortexa and make it unusable. To
    stop these kinds of attacks 1 in 10 successful transactions are charged. This
    adds up to a cost of 0.001VOA per transaction on average.
*/
...
bool CPrivateSendServer::IsInputScriptSigValid(const CTxIn& txin)
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        // TODO we're using amount=0 here but we should use the correct amount. This works because Vortexa ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
