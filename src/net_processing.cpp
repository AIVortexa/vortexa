 ...
 *  We have 4 times smaller block times in Vortexa, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Vortexa Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // disconnect nodes that arent at least 36.x series 
        if (pfrom->cleanSubVer.find("0.36") == std::string::npos) {
        ...
        // set nodes not relaying blocks and tx and not serving (parts) of the historical blockchain as "clients vertex_nodes"
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // VOA this code also handles non-TXs (Vortexa specific messages)
