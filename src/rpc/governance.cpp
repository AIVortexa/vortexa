...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 VOA irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on vortexa forum etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in vortexa.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("vortexa.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in vortexa.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Vortexa features */
    { "vortexa",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "vortexa",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "vortexa",               "gobject",                &gobject,                {} },
    { "vortexa",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
