...
# vortexa_hash
RUN git clone https://github.com/AIVortexa/vortexa_hash
RUN cd vortexa_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} vortexa
RUN useradd -u ${USER_ID} -g vortexa -s /bin/bash -m -d /vortexa vortexa
...
RUN mkdir /vortexa-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /vortexa-src && \
